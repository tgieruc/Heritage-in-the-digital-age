...
def labelwise_nms(segmentation_results, iou_th=0.2):
    ...
    for idx_, idx_tokeep, caption in zip(idx, idx_to_keep, list(unique_caption)):
        ...
        captions += (
            [segmentation_results[2][i].removeprefix(' ') for i in torch.index_select(torch.tensor(idx_), 0, idx_tokeep)])
...
def global_nms(segmentation_results, iou_th=0.9):
    ...
    captions += ([segmentation_results[2][i].removeprefix(' ') for i in idx_to_keep])
